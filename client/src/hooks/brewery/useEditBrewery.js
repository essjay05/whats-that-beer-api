...
const useEditBrewery
 = () => {
  const [error, setError] = useState(null)
  const [loading, setLoading] = useState(null )

  const updateBrewery = async (values) => {
    ...
    try {
      ...
      console.error(err)
      message.error('Brewery update failed.')
    } finally {
      setLoading(false)
    }
  }

  return { loading, error, updateBrewery }
}

export default useEditBrewery
